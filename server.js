...
io.on('connection', (socket) => {
    ...
    socket.on('find_match', (userData) => {
        ...
        if (!isAlreadyInQueue) {
            ...
            console.log(`🏎️ Növbəyə qoşuldu: @${userData.username} ${userData.user_id == ADMIN_ID ? '(ADMIN)' : ''}`);
        }
        ...
        // 🔥 YARIŞ BAŞLADICI (Admin bypass ilə)
        if (matchmakingQueue.length === 4 || matchmakingQueue.some(p => p.user_id == ADMIN_ID)) {
